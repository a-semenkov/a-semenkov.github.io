...
const fonts = () => {
  return src(`${paths.fontsFolder}/*.ttf`)
    .pipe(ttf2woff2())
...
const video = () => {
  return src([`${paths.srcVideoFolder}/**/**.{mp4,mkv}`]).pipe(
    dest(paths.buildVideoFolder)
  );
};
...
const watchFiles = () => {
  ...
  watch(`${paths.pugComponents}`, htmlInclude);
  watch(`${paths.fontsFolder}/*.ttf`, fonts);
